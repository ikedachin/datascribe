...
## 出力形式
- 同期版は `output_path`（YAML）配下に `<pdf_stem>.jsonl` を保存します。
- Async 版のファイル配置:
  - 最終出力（2 パス目の参照統合結果）: `output_path` 直下の `<pdf_stem>.jsonl`
...
- 各行には主に次のキーが入ります。
  ...
  - `objects_bbox`
  - `object_content`
  - `duplicate_check`（同期版のみ）
  - `ocr_generator`
- Async 版の最終出力にはさらに次のキーが入ります。
...
Async worker pool 版の出力 JSONL も同じ schema なので、同じように `main_2_sanitization.py` または `main_2_sanitization_async_pool.py` の入力にできます。

## Async worker pool 版
`main_1_ocr_async_pool.py` は 2 パス構成です。

1 パス目（`pipelines/ocr_model_worker_pool.py`）: ページ単位（画像 1 枚）の worker pool。各 item は同じ worker が `ocr -> objects_bbox -> object_content -> 保存` まで処理し、`tmp/<pdf_stem>_tmp.jsonl` に書き込みます。同期版と異なり 2 ページ送り・`duplicate_check` は行いません（OCR プロンプトは単ページ用 `ocr_single_page.txt` を使用）。
...
- プロンプト: `prompts`
  - `ocr_prompt`（async 版は単ページ用 `ocr_single_page.txt`、同期版はペア用 `ocr.txt`）
  - `objects_prompt`
  - `content_understanding_prompt`
  - `weave_prompt`（async 2 パス目の織り込み用）
  - `duplicate_check_prompt`（同期版のみ使用）
